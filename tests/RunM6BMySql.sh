...
m6_need() { "$@" || m6_fail prerequisite_command; }
m6_git() { git -C "$repo" "$@"; }
m6_docker() { timeout --kill-after=2s 15s env -u DOCKER_CONTEXT -u DOCKER_HOST docker --host "$endpoint" "$@"; }
...
m6_endpoint() {
    ...
    # Context metadata does not contact a daemon. Resolve overrides before any daemon call.
    context=${DOCKER_CONTEXT:-$(docker context show)}
    selected=$(docker context inspect "$context" --format '{{.Endpoints.docker.Host}}') || m6_fail docker_context;
    [[ $selected == "$endpoint" && ${DOCKER_HOST:-$endpoint} == "$endpoint" ]] || m6_fail remote_or_conflicting_endpoint;
}
m6_delegation() {
    ...
    [[ $(m6_system show docker.service -p ActiveState --value) == active && $(m6_system show docker.service -p Delegate --value) == yes ]] || m6_fail rootless_service_delegation;
    local daemon_pid
...
m6_headroom() {
    disk_floor=1073741824; memory_floor=268435456; local initial_disk=6442450944 initial_memory=3221225472
    if [[ $host_mode == shared-staging ]]; then
        initial_disk=10737418240; disk_floor=4294967296; initial_memory=4294967296; memory_floor=1610612736
        [[ $(getconf _NPROCESSORS_ONLN) -ge 4 ]] || m6_fail shared_host_cpu_headroom
    fi
    [[ $(m6_available_memory) -ge $initial_memory ]] || m6_fail memory_headroom
    [[ $(m6_free "$docker_root") -ge $initial_disk && $(m6_free "$base/evidence") -ge $initial_disk ]] || m6_fail disk_headroom
}
m6_preflight() {
    local key tool
    ...
    for tool in git docker timeout realpath stat sha256sum df du flock mktemp mkfifo systemctl systemd-run getent getconf head tail tr date sleep env hostname uname id cat rm rmdir; do command -v "$tool" >/dev/null || m6_fail missing_tool; done
    [[ $php == /* && -f $php && -x $php && $(realpath "$php") == "$php" ]] || m6_fail explicit_php_executable;
    uid=$(id -u); runtime_dir="/run/user/$uid"; m6_identity
    ...
    base="$account_home/m6b-validation"; m6_private "$base"; m6_private "$base/checkouts"; m6_private "$base/evidence"
    ...
    m6_endpoint; m6_delegation
    docker_root=$(m6_docker info --format '{{json .}}' 2>/dev/null | m6_guard engine "$platform") || m6_fail engine_not_approved
    [[ $(realpath "$docker_root") == "$docker_root" && $docker_root == "$account_home/"* && $(stat -c %u "$docker_root") == "$uid" ]] || m6_fail rootless_data_owner
...
m6_stop_unit() {
    ...
    # A never-created transient unit is safe; an inaccessible or foreign one is not.
    [[ $(m6_system show "$unit" -p LoadState --value 2>/dev/null) == not-found ]] && return 0
    m6_owned_unit && m6_system stop "$unit" >/dev/null 2>&1 || return 1
    local state remaining
    state=$(m6_system show "$unit" -p ActiveState --value 2>/dev/null) || return 1
    [[ $state == inactive || $state == failed ]] || return 1
    if [[ -n ${php_cg:-} && -e $php_cg/cgroup.procs ]]; then
        remaining=$(cat "$php_cg/cgroup.procs") || return 1
...
m6_cleanup_container() {
    ...
    m6_docker logs --tail 200 "$verified" 2>&1 | m6_redact > "$evidence/mysql-tail.txt" || true
...
m6_budget() {
    ...
    [[ $(m6_free "$docker_root") -ge $disk_floor && $(m6_free "$evidence") -ge $disk_floor && $(m6_available_memory) -ge $memory_floor ]]
}
m6_run() {
    container_attempted=0; unit_attempted=0; collector=''; supervisor=''; container_id=''; evidence=''; scratch=''; php_cg=''; failure=startup; test_exit=NOT_EXECUTED
    trap m6_finish EXIT
    ...
    exec 9> "$base/run.lock"; flock -n 9 || m6_fail another_launcher_active
    evidence=$(mktemp -d "$base/evidence/run-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXXXX")
    ...
    port=$(m6_docker inspect "$container_id" 2>/dev/null | m6_guard container) || m6_fail container_controls
    m6_cgroup
    ...
    while kill -0 "$supervisor" 2>/dev/null; do
        ...
        m6_budget || { failure=headroom_or_storage_budget_or_inspection; exit 2; }
        sleep 1
    done
...
m6_main() {
    local mode='' expected_sha='' expected_host='' expected_user='' php='' socket='' digest='' platform='' host_mode=dedicated
    ...
    while (($#)); do
        case $1 in
            ...
            --expected-sha|--expected-host|--expected-user|--php|--docker-socket|--image-digest|--platform|--host-mode)
                [[ $# -ge 2 ]] || m6_fail missing_argument
                case $1 in --expected-sha) expected_sha=$2;; --expected-host) expected_host=$2;; --expected-user) expected_user=$2;; --php) php=$2;; --docker-socket) socket=$2;; --image-digest) digest=$2;; --platform) platform=$2;; --host-mode) host_mode=$2;; esac; shift 2;;
