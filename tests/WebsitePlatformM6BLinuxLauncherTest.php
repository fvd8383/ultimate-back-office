...
$cases=['identity-valid'=>0,'identity-host'=>2,'identity-user'=>2,'identity-root'=>2,'identity-stage-default'=>2,'identity-shared'=>0,'identity-shared-user'=>2,'identity-production'=>2,'identity-production-default'=>2,
    ...
    'endpoint-valid'=>0,'endpoint-remote'=>2,'endpoint-ambiguous'=>2,'endpoint-conflict'=>2,'endpoint-rootful'=>2,'delegation-valid'=>0,'delegation-controller'=>2,'delegation-ignored'=>2,
...
foreach($cases as$case=>$expected){
    ...
    if($case==='check-only')launcherCheck($commands==="prerequisites\n",'Check-only must never enter run');
    ...
    if(str_starts_with($case,'run-')){
        launcherCheck(is_file($evidence.'/SHA256SUMS'),'Full lifecycle manifest '.$case);
        ...
        launcherCheck(str_contains($report,$case==='run-success'?'Overall: PASS':'Overall: NON_SUCCESS'),'Full lifecycle verdict '.$case);
